...
public class SignatureMath {
	...
	/**
	 * Advanced Euclidean algorithm. Search for coefficients x1, y1, d1 in the
	 * equation: x1 * a + y1 * b = d1 (d1 = GCD(a,b))
	 * 
	 * @param a first number
	 * @param b second number
	...
	public static BigInteger getMultiplicativelyInverse(BigInteger number, BigInteger phi) {
		if (!gcd(number, phi).equals(BigInteger.ONE))
			throw new ArithmeticException("Euler's function and number must be mutually simple.\n");
		if (number.compareTo(phi) > 0)
			throw new ArithmeticException("Number >= phi");
