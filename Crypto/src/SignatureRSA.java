...
public class SignatureRSA {
	 ...
	 * @param privateexp   private exponent, 1 < p < (p-1)(q-1), gcd(privateexp,
	 *                     (p-1)(q-1)) = 1
	 * @param hashFunction The object to get the hash function (Must implement the
	 *                     interface CryptoHash)
	 */
	public SignatureRSA(BigInteger p, BigInteger q, BigInteger privateexp, CryptoHash hashFunction) {
		if (!SignatureMath.isPrime(p))
			throw new ArithmeticException("P should be prime");
		if (!SignatureMath.isPrime(q))
			throw new ArithmeticException("Q should be prime");

		BigInteger r = p.multiply(q); // r = p * q
		BigInteger phi = p.subtract(BigInteger.ONE).multiply(q.subtract(BigInteger.ONE)); // phi = (p-1)(q-1)

		if ((privateexp.compareTo(BigInteger.ONE) <= 0) || (privateexp.compareTo(phi) >= 0))
			throw new ArithmeticException("D should be from 2 to phi(p*q) - 1");
		if (!privateexp.gcd(phi).equals(BigInteger.ONE))
			throw new ArithmeticException("D should be mutually simple with phi(p*q)");

		if (r.bitLength() <= hashFunction.getMinBitLength())
			throw new ArithmeticException("Insufficient size r(p*q)");

		BigInteger publicexp = SignatureMath.getMultiplicativelyInverse(privateexp, phi); // publicexp * privateexit ==
																							// 1
	...
	public static boolean checkSignature(RSAPublicKey key, BigInteger s, byte[] message, CryptoHash hashFunction) {
		BigInteger hashFromSign = SignatureMath.power(s, key.getE(), key.getR());
